...
while True:
    try:
        ...
        s3.meta.client.upload_file(upload_path, bucket_name, 'web/WebImage-'+key.split("/")[1]) #creates folder within ingest bucket
        print("Done for web/WebImage-"+str(key.split("/")[1]))
        sqs_client.delete_message(
                QueueUrl=queue_url,
                ReceiptHandle=receipt_handle
            )
